...
function toggleMode(m) {
    ...
    if (loginBox)
        loginBox.classList.toggle('hidden', m !== 'login');
    if (regBox)
        regBox.classList.toggle('hidden', m !== 'reg');
}
...
if (btnLogin) {
    btnLogin.onclick = async () => {
        ...
        if (!username || !password)
            return toast('Заполните все поля', true);
        try {
...
if (btnSendCode) {
    btnSendCode.onclick = async () => {
        ...
        if (!email)
            return toast('Введите email', true);
...
if (btnReg) {
    btnReg.onclick = async () => {
        ...
        if (!code || !username)
            return toast('Заполните код и логин', true);
        try {
