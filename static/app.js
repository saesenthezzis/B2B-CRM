...
async function loadAll() {
    renderQueues();
    try {
        const m = await fetch('/api/meta').then(r => { if (r.status === 401)
            throw new Error('401'); return r.json(); });
        META = m;
        ...
        (m.specialists || []).forEach(s => {
            if (!SPEC[s.name])
                SPEC[s.name] = [];
            if (s.city && !SPEC[s.name].includes(s.city))
                SPEC[s.name].push(s.city);
        });
        fillUserSelect();
        fillCitySelect();
        if (localStorage.getItem('fPeriod'))
            $('fPeriod').value = localStorage.getItem('fPeriod');
        if (localStorage.getItem('fStatus'))
            $('fStatus').value = localStorage.getItem('fStatus');
        if (localStorage.getItem('fPayment'))
            $('fPayment').value = localStorage.getItem('fPayment');
        ...
        if (e.message === '401')
            ...
            toast('Ошибка загрузки данных', true);
    }
}
...
function renderQueues() {
    ...
    $('queues').querySelectorAll('input[type="radio"]').forEach((b) => b.onchange = () => { queue = b.dataset.q; page = 0; debouncedRender(); });
}
...
async function render() {
    ...
    thead.innerHTML = '<tr>' + COLS.map(c => `<th data-c="${c.id}">${c.name}${c.id === sortCol ? (sortDir < 0 ? ' ▼' : ' ▲') : ''}</th>`).join('') + '</tr>';
    ...
    try {
        ...
        if (!rDeals.ok || !rSummary.ok) {
            if (rDeals.status === 401 || rSummary.status === 401)
...
async function patch(key, fields) {
    ...
    if (!user) {
        toast('Сначала выберите менеджера в шапке — без этого правки не сохраняются', true);
        render();
        return;
    }
    ...
    try {
        ...
        if (fresh.errors && fresh.errors.length)
            toast('Сохранено, но: ' + fresh.errors[0], true);
    }
    catch (e) {
        if (chk) {
...
async function renderBoss() {
    if (typeof window.initBossDashboard === 'function') {
        window.initBossDashboard();
    }
    else {
        console.warn('initBossDashboard is not defined');
    }
}
;
...
async function renderSettings() {
    ...
    try {
        ...
        tbody.innerHTML = specialists.map(s => `<tr><td>${esc(s.name)}</td><td>${esc(s.city)}</td></tr>`).join('') || '<tr><td colspan="2" style="text-align:center;color:#888;padding:22px">Нет специалистов</td></tr>';
    }
    catch (e) {
        if (e.name !== 'AbortError') {
